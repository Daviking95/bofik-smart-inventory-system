<?php
    ...
    $bname = $_POST['bname'];
    $bmonth = $_POST['bmonth'];
    $month = date("n",strtotime($bmonth));
    $res_month = date("F",strtotime($bmonth) );
    ...
    // echo "Hey";
        ...
        $sqlcomp = "SELECT DISTINCT * FROM inventory WHERE brand_name = '$bname' LIMIT 1 ";
                      ...
                      while($rowcompany=mysqli_fetch_assoc($resultcomp)){
                          ...
                          echo '<hr><div class="center-align" style="margin-top: 50px;" id="compy">'.strtoupper($res_month).' REPORT FOR '.
        ...
        echo     '<table id="data-table-row-grouping2" class="display responsive-table data-table-simple" cellspacing="0" width="100%">
                  ...
                  <tbody>';
                        // OR month(inv_date) = '$month'
                        $sqlinv = "SELECT * FROM inventory WHERE brand_name = '$bname' AND (month(pur_date) = '$month' OR month(receive_date) = '$month') ORDER BY receive_date";
              ...
              echo'<div class="right-align" style="margin-top: 20px;">
                    <form class="form-horizontal" action="functions_monthly.php" method="post" name="upload_excel" enctype="multipart/form-data">
                        <a href="../functions_monthly.php?'.$str.'" class="waves-effect waves-light btn"><i class="mdi-action-get-app right"></i>Download As CSV</a>
                    </form>
              </div>';
